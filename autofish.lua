...
-- ANDROID/DELTA COMPATIBLE - v3.0 FIXED TIMING
-- ==========================================================
...
-- FISHING TIMING (FIXED, TIDAK BISA DIUBAH)
local FISH_BITE_WAIT      = 15.0  -- Tepat 15 detik menunggu gigitan
local FISH_MINIGAME_SECS  = 8.0   -- Tepat 8 detik durasi minigame
local FISH_RECAST_DELAY   = 1.0   -- Delay sebelum cast ulang setelah berhasil
local FISH_CAST_DURATION  = 1.8   -- Durasi hold klik saat casting
...
titleLabel.Text = "AUTO FARM SYSTEM v3"
...
local fishPanelStroke = Instance.new("UIStroke", fishPanel)
fishPanelStroke.Color = Color3.fromRGB(30, 60, 100)
fishPanelStroke.Thickness = 1
...
local phaseColors = {
    ...
    Color3.fromRGB(255,  80, 200),  -- GAME  = pink
...
for i, name in ipairs(phaseNames) do
    ...
    dot.BackgroundColor3 = Color3.fromRGB(35, 35, 50)
    dot.BorderSizePixel = 0
    Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)

    local dotStroke = Instance.new("UIStroke", dot)
    dotStroke.Color = Color3.fromRGB(60, 60, 80)
...
-- Timer / progress bar for current phase
...
-- Phase active index tracker
local activeFishPhase = 0  -- 0=none, 1=CAST, 2=WAIT, 3=GAME, 4=DONE

local function setFishPhase(phaseIdx)
    if activeFishPhase == phaseIdx then return end
    activeFishPhase = phaseIdx
    for i, dot in ipairs(phaseDots) do
        local lbl = phaseLabels[i]
        if i < phaseIdx then
            -- completed
            dot.BackgroundColor3 = Color3.fromRGB(40, 100, 55)
            lbl.TextColor3 = Color3.fromRGB(80, 200, 100)
        elseif i == phaseIdx then
            ...
            local tween = TweenService:Create(dot, info, {
    ...
    timerFill.BackgroundColor3 = phaseIdx > 0 and phaseColors[math.clamp(phaseIdx,1,4)] or Color3.fromRGB(100,180,255)
end
...
statLabel.Position = UDim2.new(0, 0, 0, 266)
...
btnSmooth.Font = Enum.Font.SourceSans; btnSmooth.TextSize = 10
...
btnSpeed.Font = Enum.Font.SourceSans; btnSpeed.TextSize = 10
...
btnClearConsole.Text = "Clear"; btnClearConsole.Font = Enum.Font.SourceSans
btnClearConsole.TextSize = 9; btnClearConsole.TextColor3 = Color3.fromRGB(200,200,200)
...
consoleBox.Size = UDim2.new(1, -16, 0, 160)
...
local cs = Instance.new("UIStroke", consoleBox)
cs.Color = Color3.fromRGB(60,60,80); cs.Thickness = 1
...
local function addConsoleLog(text, color)
    ...
    if #current > 0 then current = current .. "\n" .. line else current = line end
    local lines = {}
...
btnClearConsole.MouseButton1Click:Connect(function() consoleBox.Text = "" end)
...
-- Progress bar update (called in heartbeat)
...
local function castRod()
    ...
    safeRun(function()
        ...
        -- Animate cast progress bar
        local castDuration = FISH_CAST_DURATION
        local castStart = os.clock()
        castStartTime = castStart
        while os.clock() - castStart < castDuration do
            task.wait(0.05)
            if mode ~= "FISH" then
            ...
            updateTimerFill((os.clock() - castStart) / castDuration)
        end
        ...
        -- Phase 2: WAITING for bite (exactly 15 seconds)
        if mode == "FISH" then
            ...
            warn("[FISHING] Umpan dilempar! Menunggu gigitan 15 detik...")
        end
    end)
    task.wait(0.2)
    isCasting = false
...
-- ==========================================
-- HANDLE SUCCESS (called once after minigame)
-- ==========================================
local function handleFishSuccess()
    ...
    setStatus("✅ Ikan berhasil ditangkap! (" .. tostring(fishCaughtCount) .. ")")
    warn("[FISHING] Ikan berhasil! Total: " .. tostring(fishCaughtCount))
    ...
    task.spawn(function()
        ...
        resetFishingState()
        warn("[FISHING] Recast setelah berhasil...")
        task.wait(0.1)
        task.spawn(castRod)
    end)
end
...
-- ==========================================
-- FISHING: MINIGAME BAR DETECTION
-- ==========================================
...
RunService.Heartbeat:Connect(function()
    ...
    safeRun(function()
        ...
        -- ── PHASE: WAITING (15-second countdown) ──────────────────────────
        if fishingState == "WAITING" then
            ...
            -- At exactly 15 seconds → switch to minigame phase
            if elapsed >= FISH_BITE_WAIT then
                ...
                setStatus("🎮 Minigame! (8s)")
                warn("[FISHING] 15 detik selesai → Minigame dimulai!")
                return
            end
            return
        end
        ...
        if fishingState == "MINIGAME" then
            ...
            local fraction = math.clamp(elapsed / FISH_MINIGAME_SECS, 0, 1)
            updateTimerFill(fraction)
            setStatus(string.format("🎮 Minigame! %.1fs / 8s", elapsed))

            -- After exactly 8 seconds → auto success
            if elapsed >= FISH_MINIGAME_SECS then
                setSpacePressed(false, true)
                warn("[FISHING] 8 detik minigame habis → Berhasil!")
                handleFishSuccess()
                return
            end

            -- Try to read minigame GUI bars
            ...
            if white and red and white.Visible and red.Visible then
                ...
                local isInside  = predicted >= (redLeft - tolerance) and predicted <= (redRight + tolerance)
                ...
                if isInside then
                    ...
                    elseif whiteCenter > redRight then
                        ...
                        local err = whiteCenter - (redLeft + redRight) / 2
                        if math.abs(err) > tolerance * 0.5 then setSpacePressed(err < 0) end
                    end
                else
                    local err = redCenter - predicted
                    if err > tolerance then setSpacePressed(true)
                    elseif err < -tolerance then setSpacePressed(false)
                    elseif math.abs(whiteVelocity) > 200 then setSpacePressed(whiteVelocity < 0) end
                end
            else
                -- GUI not found but timer still running: keep pressing based on velocity
                if lastWhiteCenter and math.abs(whiteVelocity) > 100 then
        ...
        -- ── PHASE: SUCCESS (handled separately, just guard) ──────────────
    ...
    while cursor and cursor ~= workspace do
        ...
        for _, childName in ipairs({"Owner","OwnerName","PlayerName","UserId"}) do
            ...
            if child and child:IsA("ValueBase") then
                ...
                if typeof(value) == "string" and value ~= "" and value ~= player.Name
                   and value ~= player.DisplayName and value ~= tostring(player.UserId) then return true end
                if typeof(value) == "Instance" and value:IsA("Player") and value ~= player then return true end
            end
        end
    ...
    for i = 1, 16 do
        ...
        if ground and ground.Instance and ground.Normal.Y > 0.55 then
            ...
            if hDelta < 10 and pos.Y - origin.Y < 4.5 then
                ...
                local sc = (pos - char.PrimaryPart.Position).Magnitude + math.abs(distC-radius)*4 + (clearance and 0 or 12) + hDelta
...
local function moveToPosition(hum, targetPos, targetPart)
    ...
    for index, wp in ipairs(waypoints) do
        ...
        while mode == "MINE" and os.clock()-started < 4.5 do
            ...
            local reach = (MINE_SMOOTH_MOVE and index < #waypoints) and 5.2 or 2.4
...
local function facePart(part)
    ...
    pcall(function() char:SetPrimaryPartCFrame(CFrame.lookAt(pos, Vector3.new(part.Position.X, pos.Y, part.Position.Z))) end)
end
...
local function mineRoutine()
    ...
    while mode == "MINE" do
        ...
        safeRun(function()
            ...
            warn("[MINE] Crystal: " .. crystal.Name .. " jarak " .. math.floor(dist))
...
warn("=== INDO HANGOUT BOT v3 LOADED ===")
warn("FISH: Cast → 15s tunggu → 8s minigame → Berhasil → Recast")
